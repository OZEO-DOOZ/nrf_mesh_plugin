...
class ProvisioningEvent extends _ProvisioningEvent {
  ...
  Stream<void> get onProvisioningCapabilities =>
      _provisioningCapabilitiesController.stream;
  Stream<void> get onProvisioningInvitation =>
      _provisioningInvitationController.stream;
  Stream<void> get onProvisioningReconnect =>
      _provisioningReconnectController.stream;
  Stream<void> get onConfigCompositionDataStatus =>
      _onConfigCompositionDataStatusController.stream;
  Stream<void> get onConfigAppKeyStatus =>
...
Future<ProvisionedMeshNode> provisioning(
  ...
  if (Platform.isIOS || Platform.isAndroid) {
    return _provisioning(
        meshManagerApi, bleMeshManager, device, serviceDataUuid, events);
  }
...
Future<ProvisionedMeshNode> _provisioning(
    ...
    String serviceDataUuid,
    ProvisioningEvent events) async {
  assert(meshManagerApi.meshNetwork != null,
  ...
  final onProvisioningCompletedSubscription =
    ...
    while (scanResult == null) {
      ...
      scanResult = scanResults.firstWhere(
          (element) => element.device.id.id == device.id.id,
          orElse: () => null);
  ...
  final onProvisioningFailedSubscription =
      meshManagerApi.onProvisioningFailed.listen((event) async {
    await meshManagerApi.cleanProvisioningData();
    completer
        .completeError(Exception('Failed to provision device ${device.id.id}'));
  });

  final onProvisioningStateChangedSubscription =
    ...
    if (event.state == 'PROVISIONING_CAPABILITIES') {
      ...
      if (Platform.isAndroid) {
        ...
        var unicast = await meshManagerApi.meshNetwork
            .nextAvailableUnicastAddress(elementSize);
  ...
  final onDeviceReadySubscription =
  ...
  final onDataReceivedSubscription =
  ...
  final onConfigAppKeyStatusSubscription =
